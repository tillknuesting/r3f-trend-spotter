...
function App() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // In production, this file is generated by the agent daily.
      // During development, we need to ensure main.go runs to create it.
      const res = await fetch("/data/signals.json");
      if (!res.ok) throw new Error("Failed to load signals.json");
  ...
  return (
    <div className="min-h-screen bg-background text-primary selection:bg-accent selection:text-black pt-1">
      ...
      <footer className="border-t border-gray-200 bg-slate-50 py-12">
        <div className="container mx-auto max-w-7xl px-6 flex flex-col sm:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            <div className="h-4 w-4 bg-black" />
            <span className="font-bold tracking-tight">R3F ENGINEERING</span>
          </div>
          <p className="font-mono text-xs text-gray-400">
            GENERATED BY R3F-TREND-SPOTTER-V3 // {new Date().getFullYear()}
          </p>
        </div>
      </footer>
    </div>
